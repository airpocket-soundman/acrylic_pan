...
起動時は停止状態です。`CAPTURE` は2,048点を取得して送信後に停止へ戻るため、繰り返し安全に呼べます。通常の衝撃収録は `START` で待機し、64点（2.5 ms）のプリトリガを含む2,048点（80 ms）を返します。推論モードだけは現行モデル互換の512点を使用します。±32 g設定ではjerkしきい値1,000 LSB、levelしきい値200 LSBです。

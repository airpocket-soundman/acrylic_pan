...
Position-probability mode (`mode=3`) runs a 128-32-60 Solist-AI model as five
12-output heads and performs a temperature-0.05, 60-way softmax on the MCU.
Alpha is the fixed seed-1 projection generated by `ODL_Initialize`; only the
distilled Beta weights are deployed. It sends `POSITION_RESULT`
(`0x24`) with normalized float32 probabilities and the measured Solist,
softmax, and combined compute times. No waveform is sent; the PC only
aggregates and renders the device-generated distribution. This mode is for the
400 x 300 x 5 mm panel and its 60 measured positions.

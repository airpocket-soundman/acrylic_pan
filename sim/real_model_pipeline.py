"""Train the measured-vibration Acrylic Pan eight-class Solist-AI model.
...
def extract_hybrid_features(samples: np.ndarray) -> np.ndarray:
    ...
    if waveform.ndim != 1 or waveform.size != SAMPLE_COUNT:
        raise ValueError(f"waveform must contain exactly {SAMPLE_COUNT} samples")
...
def load_hybrid_dataset(source: Path):
    """Load validated Recorder sessions, replacing FFT-only features."""
    dataset = load_recorded_sessions(source, require_all_classes=True)
    features = []
...
def confusion_matrix(expected: np.ndarray, predicted: np.ndarray) -> np.ndarray:
...
def evaluate_fold(features: np.ndarray, labels: np.ndarray, session_ids: np.ndarray,
                  ...
                  ridge: float = RIDGE) -> tuple[dict[str, Any], FeatureScaler, np.ndarray]:
    ...
    beta = fit_beta(train_x, labels[train], alpha, ridge)
    scores = mcu_reference(test_x, alpha, beta)
    predicted = np.argmax(scores, axis=1)
    matrix = confusion_matrix(labels[test], predicted)
...
def export_header(path: Path, alpha: np.ndarray, beta: np.ndarray,
                  scaler: FeatureScaler, golden_inputs: np.ndarray) -> None:
...
"""
    ...
    content += _c_bfloat_array(
        ...
        "apan_model_golden_inputs[8][APAN_MODEL_INPUT_SIZE]",
    )
    content += "#endif\n"
...
def generate(source: Path, output_dir: Path, header: Path,
             alpha_path: Path = DEFAULT_ALPHA, ridge: float = RIDGE) -> dict[str, Any]:
    dataset, features = load_hybrid_dataset(source)
    ...
    for fold_index, (train_session, test_session) in enumerate(
        ...
        result, scaler, _ = evaluate_fold(
            ...
            train_session, test_session, alpha, ridge,
        )
        folds.append(result)
        ...
        export_solist_csv(
            output_dir / f"fold{fold_index}_train_8class.csv",
            scaler.transform(features[train]), one_hot(dataset.labels[train]),
        )
        export_solist_csv(
            output_dir / f"fold{fold_index}_test_8class.csv",
            scaler.transform(features[test]), one_hot(dataset.labels[test]),
        )

    final_scaler = FeatureScaler.fit(features)
    final_x = final_scaler.transform(features)
    final_beta = fit_beta(final_x, dataset.labels, alpha, ridge)
    final_scores = mcu_reference(final_x, alpha, final_beta)
    final_predictions = np.argmax(final_scores, axis=1)
    final_matrix = confusion_matrix(dataset.labels, final_predictions)
    ...
    export_solist_csv(
        output_dir / "final_train_8class.csv", final_x, one_hot(dataset.labels)
    )
    export_solist_csv(
        output_dir / "final_parity_8class.csv", golden_inputs,
        one_hot(np.arange(CLASS_COUNT, dtype=np.int64)),
    )
    export_header(header, alpha, final_beta, final_scaler, golden_inputs)

    model_hash = hashlib.sha256((output_dir / "model.npz").read_bytes()).hexdigest()
    report = {
        "model": "acrylic_pan_time128_h32_8class_v1",
        "sessions": sessions,
